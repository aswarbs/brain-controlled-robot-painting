...
def update_buffer(data_buffer, new_data, notch=False, filter_state=None):
    ...
    num_cols = max(data_buffer.shape[1], new_data.shape[1])
    if data_buffer.shape[1] < num_cols:
        data_buffer = np.pad(data_buffer, ((0, 0), (0, num_cols - data_buffer.shape[1])), mode='constant')
    elif new_data.shape[1] < num_cols:
        new_data = np.pad(new_data, ((0, 0), (0, num_cols - new_data.shape[1])), mode='constant')
